        ...
        if reqs.empty: st.info("No pending requests")
            ...
            for i, region in enumerate(regions):
                with region_tabs[i]:
                    ...
                    with st.form(f"mgr_req_form_{region}"):
            ...
            if len(regions) > 0:
                ...
                for i, region in enumerate(regions):
                    with rtabs[i]:
                        ...
                        with st.form(f"sk_issue_form_{region}"):
                            ...
                            if st.form_submit_button(f"Confirm Bulk Issue for {region}", use_container_width=True):
                                issued_count = 0
...
def supervisor_view_warehouse():
    ...
    with t1: # Bulk Request
        ...
        if not inv.empty:
            ...
            with st.form(f"sup_order_form_{selected_region_wh}"):
                ...
                if st.form_submit_button(txt['send_req'], use_container_width=True):
    ...
    with t2: # Ready for Pickup
        ...
        if ready.empty: st.info(f"No items ready for pickup in {selected_region_wh}.")
            ...
            with st.form(f"sup_pickup_form_{selected_region_wh}"):
                ...
                if st.form_submit_button(f"Confirm Receipt for {selected_region_wh}", use_container_width=True):
                    rec_count = 0
    ...
    with t3: # Edit Pending
        ...
        if pending.empty: st.info(f"No pending requests for {selected_region_wh}.")
            ...
            with st.form(f"sup_edit_pending_{selected_region_wh}"):
    ...
    with t4: # Local Inventory
        ...
        if local_inv.empty:
            ...
            with st.form(f"local_inv_form_{selected_region_wh}"):
                ...
                if st.form_submit_button(f"Update {selected_region_wh} Counts", use_container_width=True):
                    up_count = 0
